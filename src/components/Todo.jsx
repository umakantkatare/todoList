    ...
    <>
      <div className=" container bg-violet-200 mx-auto rounded-md my-5 p-5 min-h-[80vh] w-full ">
        ...
        <div>
         ...
         <div className="flex">
          <input type="checkbox"  id={item.id} checked={item.isCompleted} onChange={handleCheckBox} />
             <div>{item.todo} </div>
              <div className="button">
                <button onClick={(e) => handleEdit (e, item.id)} className=" bg-blue-600 hover:bg-blue-700 p-2 rounded-md mx-4 text-sm text-white font-semibold py-1">Edit</button>
                <button onClick={(e) => handleDelete (e, item.id)} className=" bg-blue-600 hover:bg-blue-700 p-2 rounded-md mx-3 text-sm text-white font-semibold py-1">Delete</button>
              </div>
            </div>
            </div>
             }) }
